...
import { NotFoundException, PreconditionFailedException, HttpException } from '@nestjs/common';
...
it('should throw PreconditionFailedException if userReserve is not found', async () => {
    ...
    await expect(
        service.findReserveFromUser('userId', '1'),
...
it('should throw NotFoundException if user is not found', async () => {
    ...
    await expect(service.findReservesFromUser('userId')).rejects.toThrow(NotFoundException);
});
...
it('should throw HttpException if no reserves are found', async () => {
    ...
    await expect(service.findReservesFromUser('userId')).rejects.toThrow(HttpException);
});
...
it('should return sorted user reserves', async () => {
    const mockUser = {
        id_google: 'userId',
        userReserves: [
            { reserve: { hour_start: new Date('2023-12-01T10:00:00Z'), hour_end: new Date(Date.now() + 86400000) } }, 
            { reserve: { hour_start: new Date('2023-11-01T10:00:00Z'), hour_end: new Date(Date.now() + 86400000) } },
        ],
    };
    ...
    expect(result[0].reserve.hour_start).toEqual(new Date('2023-11-01T10:00:00Z'));
    expect(result[1].reserve.hour_start).toEqual(new Date('2023-12-01T10:00:00Z'));
...
it('should throw PreconditionFailedException if userReserve is not found', async () => {
    ...
    await expect(
        service.deleteReserveFromUser('userId', '1'),
    ).rejects.toThrow(PreconditionFailedException);
});
...
it('should not send notifications if no other users are in the reserve', async () => {
    ...
    expect(mockFcmNotificationService.sendMulticastNotification).not.toHaveBeenCalled();
});
...
it('should throw NotFoundException if reserve is not found during deletion', async () => {
    ...
    await expect(service.deleteReserveFromUser('userId', '1')).rejects.toThrow(NotFoundException);
...
it('should handle errors when sending notifications during deletion', async () => {
    ...
    expect(mockUserReserveRepository.remove).toHaveBeenCalled();
    expect(mockFcmNotificationService.sendMulticastNotification).toHaveBeenCalled();
    expect(mockUserReserveRepository.remove).toHaveBeenCalled();
    expect(mockFcmNotificationService.sendMulticastNotification).toHaveBeenCalled();
});
...
it('should not remove userReserve if it does not exist', async () => {
    ...
    await expect(service.deleteReserveFromUser('userId', '1')).rejects.toThrow(PreconditionFailedException);
    expect(mockUserReserveRepository.remove).not.toHaveBeenCalled();
});
