...
describe('GamesService', () => {
    ...
    beforeEach(async () => {
        ...
        gameCategoryRepository = module.get<Repository<GameCategoryEntity>>(getRepositoryToken(GameCategoryEntity));
        httpService = module.get<HttpService>('Bgg-Api');
    });

    it('should return all games', async () => {
        const mockGames = [
            { id_game: 1, name: 'Chess', description: 'A game', bgg_id: 123, gameCategory: null, shop: null, game_reserve: null },
        ];
    ...
    it('should return a game by id', async () => {
        const mockGame = { id_game: 1, name: 'Chess', description: 'A game', bgg_id: 123, gameCategory: null, shop: null, game_reserve: null };
    ...
    it('should create a new game', async () => {
        const createDto = { name: 'Chess', description: 'A game', bgg_id: 123, category_name: 'Strategy', shop_id: 1, reserve_id: 1, };
        const mockGame = { id_game: 1, ...createDto, gameCategory: null, shop: null, game_reserve: null };
        ...
        jest.spyOn(gameCategoryRepository, 'findOne').mockResolvedValue({ id: 1, name: 'Strategy' } as any);
    ...
    it('should throw BadRequestException when creating a game with invalid category', async () => {
        const createDto = { name: 'Chess', description: 'A game', bgg_id: 123, category_name: 'Invalid' };
    ...
    it('should update an existing game', async () => {
        const updateDto = { name: 'Updated Chess', description: 'Updated game', category_name: 'Strategy' };
        const mockGame = { id_game: 1, name: 'Chess', description: 'A game', bgg_id: 123, gameCategory: null, shop: null, game_reserve: null };
        jest.spyOn(gameRepository, 'findOne').mockResolvedValue(mockGame);
        ...
        jest.spyOn(gameRepository, 'save').mockResolvedValue({ ...mockGame, ...updateDto });
    ...
    it('should delete a game', async () => {
        jest.spyOn(gameRepository, 'delete').mockResolvedValue({ affected: 1 } as any);
    ...
    it('should throw NotFoundException when deleting a non-existent game', async () => {
        jest.spyOn(gameRepository, 'delete').mockResolvedValue({ affected: 0 } as any);
    ...
    it('should fetch games from external API when not found locally', async () => {
        ...
        const mockExternalGames = {
            data: {
                items: [
                    { objectid: 1, name: 'External Chess', image: 'image_url' },
                ],
            },
        };
