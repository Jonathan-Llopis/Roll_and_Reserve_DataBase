...
describe('ReservesService', () => {
  ...
  beforeEach(async () => {
    ...
    reserveRepository = module.get<Repository<ReservesEntity>>(
      getRepositoryToken(ReservesEntity),
    );
    ...
    difficultyRepository = module.get<Repository<DifficultyEntity>>(
      getRepositoryToken(DifficultyEntity),
    );
    tableRepository = module.get<Repository<TablesEntity>>(
      getRepositoryToken(TablesEntity),
    );
    fcmNotificationService = module.get<FcmNotificationService>(
  ...
  it('should update an existing reserve', async () => {
    ...
    jest.spyOn(reserveRepository, 'findOne').mockResolvedValue(mockReserve);
    jest
      .spyOn(reserveRepository, 'merge')
      .mockImplementation(
        (entity: ReservesEntity, dto: Partial<ReservesEntity>) => {
          Object.assign(entity, dto);
          return entity as ReservesEntity;
        },
      );
    jest
      .spyOn(reserveRepository, 'save')
      .mockResolvedValue({ ...mockReserve, ...updateDto });
  ...
  it('should throw NotFoundException when updating a non-existent reserve', async () => {
    ...
    await expect(
      service.updateReserve({ total_places: 20 }, 999),
    ).rejects.toThrow(new NotFoundException('Reserve not found'));
  });

  it('should delete a reserve', async () => {
    jest
      .spyOn(reserveRepository, 'delete')
